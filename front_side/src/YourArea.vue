  ...
  <div class="user-area-wrap">
    <div class="table-wrap">
      <table>
        <tr>
          <td></td>
          <td v-for="letter in letters"><div class="letter">{{ letter.toUpperCase() }}</div></td>
        </tr>
        <tr v-for="(row, index) in area">
          <td><div class="number">{{ index+1 }}</div></td>
          <td v-for="(field, f_index) in row" class="cell-wrap"><div class="cell"
                                                          :class="{ 'ship': field == 1, 'select': field == 4 }"
                                                          @mousedown.prevent="mouseDown(f_index, index)"
                                                          @mouseup.prevent="mouseUp"
                                                          @mouseenter="mouseEnter(f_index, index)"></div></td>
        </tr>
      </table>
    </div>
  </div>
    ...
    methods: {
      ...
      mouseEnter(x, y) {
        if ( this.isClicked ) {
          ...
          if (this.canBuild) {
            EventBus.$emit('select-deselect-field', { x: x, y: y });
          }
        }
      }
    }
